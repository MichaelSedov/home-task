...
<div class="mx-auto max-w-7xl px-4 py-8 sm:px-6">
	...
	<div class="mb-6 flex flex-wrap gap-3">
		<input
			...
			class="w-56 rounded-[var(--radius-sm)] border border-[var(--border)] bg-[var(--bg-elev)] px-3 py-2 text-sm text-[var(--fg)] placeholder:text-[var(--muted)] outline-none focus:border-[var(--accent)] focus:ring-1 focus:ring-[var(--accent)]"
	...
	>
		<table class="w-full text-sm" role="grid" aria-label={t('dashboard.items.title')}>
			<thead>
				...
				>
					{#each [{ key: 'name', label: t('dashboard.items.column.name') }, { key: 'status', label: t('dashboard.items.column.status') }, { key: 'channel', label: t('dashboard.items.column.channel') }, { key: 'budget', label: t('dashboard.items.column.budget') }, { key: 'spent', label: t('dashboard.items.column.spent') }, { key: 'ctr', label: t('dashboard.items.column.ctr') }, { key: 'updatedAt', label: t('dashboard.items.column.updated') }] as col (col.key)}
						<th
							scope="col"
							aria-sort={q.sort === col.key
								? q.order === 'asc'
									? 'ascending'
									: 'descending'
								: 'none'}
